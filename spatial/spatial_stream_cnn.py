...
def test(model, nb_epoch, spatial_test_data, chunk_size, nb_classes, img_rows, img_cols, batch_size):
    ...
    random.shuffle(keys)
    X_test, Y_test = getTestData(spatial_test_data.keys()[
                                 :500], nb_classes, img_rows, img_cols)
...
def train(model, nb_epoch, spatial_train_data, spatial_test_data, chunk_size, nb_classes, img_rows, img_cols, batch_size):
    ...
    '''X_sample, Y_sample = getSampleData(spatial_train_data.keys()[
                                       :chunk_size], nb_classes, img_rows, img_cols)
    ...
    for e in range(nb_epoch):
        ...
        '''X_test, Y_test = test(model, nb_epoch, spatial_test_data,
                              chunk_size, nb_classes, img_rows, img_cols, batch_size)'''

        keys = spatial_train_data.keys()
        random.shuffle(keys)
        for chunk in chunks(keys, chunk_size):
            X_chunk, Y_chunk = getTrainData(
                chunk, nb_classes, img_rows, img_cols)
            if (X_chunk != None and Y_chunk != None):
                # for X_batch, Y_batch in datagen.flow(X_chunk, Y_chunk,
                # batch_size=chunk_size):
                '''loss = model.fit(X_chunk, Y_chunk, verbose=1, batch_size=batch_size,
                                 nb_epoch=1, show_accuracy=True, validation_data=(X_test, Y_test))'''
                loss = model.fit(X_chunk, Y_chunk, verbose=1, batch_size=batch_size,
                                 nb_epoch=1, show_accuracy=True, validation_split=0.1)                 
                instance_count += chunk_size
                print instance_count
                if instance_count % 256 == 0:
                    model.save_weights(
                        'spatial_stream_model.h5', overwrite=True)
...
if __name__ == "__main__":
    ...
    train(model, nb_epoch, spatial_train_data, spatial_test_data,
          chunk_size, nb_classes, img_rows, img_cols, batch_size)
